...
class Writer {
  ...
  writeOpenApiFile () {
    ...
    const { toOpenApi } = require('./to_openapi');
    let openApiPath;
    if (typeof this.opt.openapi === 'string') {
      if (this.path.isAbsolute(this.opt.openapi) || this.opt.openapi.includes('/') || this.opt.openapi.includes('\\')) {
        openApiPath = this.path.resolve(this.opt.openapi);
      } else {
        openApiPath = this.path.join(this.opt.dest, this.opt.openapi);
      }
    } else {
      openApiPath = this.path.join(this.opt.dest, 'openapi.json');
    }

    this.createDir(this.path.dirname(openApiPath));
    this.log.verbose('Saving OpenAPI spec to: ' + openApiPath);
    const openApiData = toOpenApi(this.api.data, this.api.project);
    this.fs.writeFileSync(openApiPath, openApiData + '\n');
  }
