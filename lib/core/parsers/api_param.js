...
function parse (content, source, defaultGroup) {
  ...
  // Pattern A: [tag](group)
  let tagMatch = content.match(/^\[([^\]\s]+)\]\s*\(([^)]+)\)/);
  if (tagMatch) {
    tag = tagMatch[1];
    content = content.replace(/^\[[^\]\s]+\]\s*/, '');
  } else {
    // Pattern B: (group)[tag]
    tagMatch = content.match(/^\(([^)]+)\)\s*\[([^\]\s]+)\]/);
    if (tagMatch) {
      tag = tagMatch[2];
      content = content.replace(/\s*\[[^\]\s]+\]/, '');
    } else {
      ...
      if (tagMatch) {
        ...
        if (tagMatch) {
          ...
          // Pattern E: [tag] fieldName (only if source starts with @api...[)
          if (source && /^@api\w+\[/.test(source)) {
            tagMatch = content.match(/^\[([^\]\s]+)\]/);
            if (tagMatch) {
              tag = tagMatch[1];
              content = content.replace(/^\[[^\]\s]+\]\s*/, '');
            }
  ...
  if (tag !== undefined) {
    if (tag.indexOf(':') !== -1) {
      ...
      result.tagType = tag.substring(index + 1);
      result.tag = tag.substring(0, index);
    } else {
      result.tag = tag;
    }
  }
